...
class OptimalKClassifier(nn.Module):
    def __init__(self, num_k_classes, hidden_dim=5):
        """
...
class WOKNN(BaseModel):
    def __init__(self, K_range=[2, 7], device="cpu", seed=None):
        """
        ...
        self.opt_k_model = OptimalKClassifier(num_k_classes=K_range[1]-K_range[0]+1).to(device)
        self.device = device
        self.seed = seed
